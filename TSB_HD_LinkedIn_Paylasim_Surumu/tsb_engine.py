...
def _mali_net_profit_records(book: XlsxRaw) -> dict[str, float]:
    """Read account 69 (Dönem Net Karı Veya Zararı) from the MALI sheet.

    Net profit is a company/sector total metric; it is not allocated to branches.
    """
    rows = dict(book.rows("MALI"))
    account_row = rows.get(5, {})
    header_row = rows.get(6, {})
    net_profit_idx = next(
        (idx for idx, value in account_row.items() if _canonical_account(value) == "69"),
        None,
    )
    if net_profit_idx is None:
        net_profit_idx = next(
            (idx for idx, value in header_row.items() if str(value or "").strip().lower() == "dönem net karı veya zararı"),
            None,
        )
    if net_profit_idx is None:
        raise KeyError("MALI: 69 Dönem Net Karı Veya Zararı sütunu bulunamadı.")
    ...
    for row_no, cells in rows.items():
        ...
        if code in (None, "") or company_type not in ("HD", "T (HD)"):
            continue
        try:
            value = float(cells.get(net_profit_idx) or 0.0)
        except (TypeError, ValueError):
            value = 0.0
        output[normalize_code(code)] = value
    return output
...
def build_period_from_income(income_bytes: bytes) -> tuple[str, str, list[dict[str, Any]]]:
    ...
    net_profit_records = _mali_net_profit_records(book)
    for code, value in net_profit_records.items():
        if code in by_sheet["HAYATDISI"]:
            by_sheet["HAYATDISI"][code]["Dönem Net Kâr/Zarar (TL)"] = value
...
def attach_net_claim_metrics(master_rows: list[dict[str, Any]], claims_bytes: bytes) -> None:
    """Enrich rows with net earned premium and DERK-included net loss ratio.

    The Hasar-Prim workbook is the authoritative source for these net columns.
    Expense/net-earned and net combined are then calculated using the already
    reconciled 614 expense from the income/expense sources.
    """
    book = XlsxRaw(claims_bytes)
    row_map = {(normalize_code(r["Şirket Kodu"]), r["Branş"]): r for r in master_rows}
    for sheet_name in REQUIRED_SHEETS:
        rows = dict(book.rows(sheet_name))
        parent_row = rows.get(5, {})
        header_row = rows.get(6, {})
        net_earned_idx = next((idx for idx, value in parent_row.items() if str(value or "").strip() == "Kazanılmış Prim (Net)"), None)
        net_incurred_idx = next((idx for idx, value in parent_row.items() if str(value or "").strip() == "Gerçekleşen Hasar (Net)"), None)
        derk_in_idx = next((idx for idx, value in parent_row.items() if str(value or "").strip() == "Derk Dahil"), None)
        derk_out_idx = next((idx for idx, value in parent_row.items() if str(value or "").strip() == "Derk Hariç"), None)
        net_hp_idx = None
        if derk_in_idx is not None:
            upper = derk_out_idx if derk_out_idx is not None else derk_in_idx + 3
            net_hp_idx = next((idx for idx, value in header_row.items() if derk_in_idx <= idx < upper and str(value or "").strip() == "Net Hasar Prim Oranı (%)"), None)
        if None in (net_earned_idx, net_incurred_idx, net_hp_idx):
            raise KeyError(f"{sheet_name}: net hasar/prim kontrol sütunları bulunamadı.")
        ...
        for row_no, cells in rows.items():
            ...
            if code in (None, "") or company_type not in ("HD", "T (HD)"):
                continue
            key = (normalize_code(code), sheet_name)
            target = row_map.get(key)
            if target is None:
                continue
            net_earned = float(cells.get(net_earned_idx) or 0.0)
            net_incurred = -float(cells.get(net_incurred_idx) or 0.0)
            net_hp = float(cells.get(net_hp_idx) or 0.0)
            expense = float(target.get("Faaliyet Gideri (TL, işaretli)") or 0.0)
            net_expense = (-expense / net_earned) if abs(net_earned) > 1e-12 else None
            target["Net Kazanılmış Prim (TL)"] = net_earned
            target["Net Gerçekleşen Hasar (TL)"] = net_incurred
            target["Net H/P (DERK Dahil)"] = net_hp
            target["Net Masraf Oranı"] = net_expense
            target["Net Bileşik Oran"] = None if net_expense is None else net_hp + net_expense
...
def prepare_import(files: Iterable[tuple[str, bytes]], tolerance_tl: float = 1.0) -> dict[str, Any]:
    ...
    attach_net_claim_metrics(master_rows, claims_bytes)
...
def save_history(active_path: Path, rows: list[dict[str, Any]], source: str = "TSB HD Streamlit v6") -> None:
    active_path.parent.mkdir(parents=True, exist_ok=True)
    payload = {"schema_version": 2, "source": source, "updated_at": datetime.now().isoformat(timespec="seconds"), "rows": rows}
...
def history_to_json_bytes(rows: list[dict[str, Any]]) -> bytes:
    payload = {"schema_version": 2, "exported_at": datetime.now().isoformat(timespec="seconds"), "rows": rows}
